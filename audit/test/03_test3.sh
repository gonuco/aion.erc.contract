...
var startSavings2Tx = savings.start($STARTTIME, {from: contractOwnerAccount, gas: 100000});
while (txpool.status.pending > 0) {
}
...
failIfGasEqualsGasUsed(startSavings2Tx, setupSavingsMessage + " - start($STARTTIME $STARTTIME_S)");
printControllerContractDetails();
printLedgerContractDetails();
printTokenContractDetails();
printSavingsContractDetails();
